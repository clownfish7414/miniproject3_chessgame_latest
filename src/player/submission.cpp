...
void write_valid_spot(std::ofstream& fout) {//可以參考這個去改
  ...
  while(true) {
    ...
    //int want_state=Alpha_Beta::get_move(root, 6, -INT_MAX, INT_MAX ,1-root->player);
    //Move move=Alpha_Beta::get_want_move(want_state,root);

    Move move=Alpha_Beta::get_move(root,depth,root->player);
